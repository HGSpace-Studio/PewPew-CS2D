  ...
  { 
   ...
   ((Graphics2D)g).setRenderingHint(RenderingHints.KEY_ANTIALIASING, 
	...
	if(showFPS)
	{
		...
		g.drawString("实体数: " + ents.size(), 20, 80);
		g.drawString("区块数: " + chunkMap.size(), 20, 100);
		g.drawString("当前区块: (" + currentChunk.getX() + "," + currentChunk.getY() + ")", 20, 120);
	}
	...
	public void drawAmmo(Graphics g)
	{
	  	if(show)
	  	{
	  		...
	  		g.setColor(Color.BLACK);
	  		g.drawString("弹匣: " + ent.getGun().magazines, 1150, 680);
		g.drawString("击杀: " + Player.kills, 1150, 20);	
	  	}	
	}
	public void drawInfoBox(Graphics g)
			...
			return;
	  		...
	  		if(selected instanceof Bot || selected instanceof Player)
	  		{
				...
				g.fillOval(cew + 10 + 40 - 5, rew + 30 - 5,10, 10);
	  			...
	  			if(selected.isGone())
					g.drawString("死亡", cew + 110, rew + 20);
				else
					g.drawString("" + l + " 秒", cew + 110, rew + 20);
				
				g.drawString("生命值: ", cew + 60, rew + 40);
				
				g.drawString("护甲: ", cew + 60, rew + 60);
	  			g.setColor(Color.RED);
	 		...
	 		else if(selected instanceof Ammo)
	 		{
	 			...
	 			g.setColor(Color.BLACK);
	  			g.drawString(selected.name, cew + 60, rew + 20);
				g.drawString("增加一个弹匣", cew + 60, rew + 40);
				g.drawString("到当前武器", cew + 60, rew + 50);
				...
				int l = (int)(selected.life/25);
				if(selected.isGone())
					g.drawString("已拾取", cew + 110, rew + 20);
				else
					g.drawString("" + l + " 秒", cew + 110, rew + 20);				
 	...
 	public void killBots()
 	{
		...
		{
			print("警告 - 可能存在数据流动延迟。(ConcurrentModificationException - 实体计算/移除可能已排队到稍后更新。");
		}
		print("" + am + " 个机器人已被击杀。");
	}
	...
	public void toggleOneHit()
	{
  	...
  	public void handleCommand()
  	{
  		...
  		if(command.contains("SPAWN") && command.indexOf("SPAWN") == 0)
  		{
  			...
  			if(spaces == 0)
  			{
  				print("未指定数量或半径。使用默认值...(10个机器人，300半径)。");
				spawnBots(10, 300);
				print("10个机器人已在玩家周围300半径范围内生成。");
  			}
  			else if(spaces == 2)
  			{
  				...
  				{
  					print("'spawn'命令格式错误。正确格式为: spawn <机器人数量> <半径>)");
				return;
			}
			spawnBots(val1, val2);
			print("" + val1 + "个机器人已在玩家周围" + val2 + "半径范围内生成。");
			return;
		...
		else if(command.contains("TELEPORT") && command.indexOf("TELEPORT") == 0)
		{
			...
			if(pieces.length == 3)
			{
				...
				int val2;
  				...
  				{
  					print("'teleport'命令格式错误。正确格式为: teleport <列> <行>)");
  					return;
  				}
  				teleport(val2, val1);
  				camera.focus(ent);
